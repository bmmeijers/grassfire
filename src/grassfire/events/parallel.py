...
# Parallel
# -----------------------------------------------------------------------------
def handle_parallel_fan(fan, pivot, now, skel, queue):
    ...
    if len(fan) == 1:
        ...
        if t.neighbours.count(None) == 3:
            logging.debug('3-triangle handling in parallel fan')
            # take edge e
            ...
            logging.debug('*NOT A* 3-triangle handling in parallel fan')
        ...
        # the newly created vertex again is infinitely fast
        if kv.inf_fast:
            handle_parallel_fan(new_fan, kv, now, skel, queue)
    else:
        return
