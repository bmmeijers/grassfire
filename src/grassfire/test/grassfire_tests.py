...
def make_test_cases(fixtures):
    ...
    for i, f in enumerate(fixtures):
        ...
        assert f.__name__ not in cases, "duplicate test name ({}) found".format(f.__name__)
        cases[f.__name__] = ('* {:>2d}: '.format(i) + str(f.__doc__), data, total, node, infinite)
    return cases

#CASES = make_test_cases([all_tests()[11]])
#INTERACTIVE = True

CASES = make_test_cases(all_tests())
INTERACTIVE = False

#CASES = make_test_cases([all_tests()[52]])
#INTERACTIVE = False
...
class TestSequenceMeta(type):
    ...
    def __new__(mcs, name, bases, dict):
        ...
        def gen_test(description, data, total, node, infinite):
            def test(self):
                if INTERACTIVE:
                    skel = calc_skel(data, pause=True, output=True, internal_only=False, shrink=True)
                else:
                    skel = calc_skel(data)
                ...
                not_stopped = filter(
                    lambda v: v.stops_at is None, skel.vertices)
                stopped = filter(
                    lambda v: v.stops_at is not None, skel.vertices)
                ...
                self.assertFalse(segments_intersecting(skel.segments()), "intersection between straight skeleton segments found")
                ...
                if True:
                    ...
                    offset_segments = [(line[0], line[1]) for line in calc_offsets(skel, last_evt_time)]
                    self.assertFalse(segments_intersecting(offset_segments), "Intersection in offsets found")
