...
class TestCollapseTime3TriangleAndSurroundings(unittest.TestCase):
    ...
    def test_inf_triangle_b(self):
        evt = compute_event(self.triangles[139742234433872])#, now=0.)
        print evt
        assert evt.time == None

    def test_inf_triangle_c(self):
        evt = compute_event(self.triangles[139742234433616])#, now=0.)
        print evt
        assert evt.time == None
